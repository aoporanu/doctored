...
$(document).ready(function() {
    //$('.fullday-block select').prop('disabled',true);
    $.validator.addMethod("needsSelection", function(value, element) {
        return $(element).multiselect("getChecked").length > 0;
    });
    $('#slot_management_not_using').validate({
        rules: {
            consulations: "required needsSelection"
...
function validateStepData(stepNumber) {
    ...
    if (stepNumber == 0) {
        ...
        if ($('#exampleFormControlSelect1').val() == '') {
            setErrorMessage('Please select Hospital');
            return true;
        } else {
            return false;
        }
    ...
    if (stepNumber == 1) {
        if (($('#consulations').val().length) == 0) {
            setErrorMessage('Please Choose consultation type');
            return true;
        }
        return false;
    }
    if (stepNumber == 2) {
        if (($('#timeval_status').val()) == 'fail') {
            // setErrorMessage('Please Choose consultation type');
            return true; 
        }else{
